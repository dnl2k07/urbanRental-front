...
export default function ReservationDetail() {
  ...
  useEffect(() => {
    async function loadCar() {
      try {
        ...
        // If user is not logged in, don't fetch cars (backend requires auth)
        if (!user || !user.user_id) {
          setError("Please log in to view car details and make reservations");
          setLoading(false);
          return;
        }
        ...
        const foundCar = rawCars.find(
          (car) => car.vehicle_id === parsedVehicleId,
        );
  ...
  async function handleProcessPayment(paymentData) {
    try {
      ...
      const res = await fetch(
        "https://nodejs208.dszcbaross.edu.hu/api/payments/process",
        {
          method: "POST",
          headers: {
  ...
  async function createReservation(transactionData) {
    try {
      const res = await fetch(
        "https://nodejs208.dszcbaross.edu.hu/users/newreservation",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
