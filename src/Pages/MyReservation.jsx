...
export default function MyReservations() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch(`https://nodejs208.dszcbaross.edu.hu/users/updatereservation`, {
        method: 'PUT',
        credentials: 'include',
        headers: {
  ...
  return (
    <>
      <div style={{marginTop: 100 , display: 'flex', flexDirection: 'column', minHeight: '50vh'}}>
        ...
        <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto' }}>
          ...
          {loading ? (
            ...
            <div style={{
              ...
              <h3>You don't have a current reservation, pick one up!</h3>
            </div>
          ) : (
            <>
              <table style={{
                ...
                <thead>
                  <tr style={{ backgroundColor: '#e6682d', color: '#fff' }}>
                    <th style={{ padding: '15px', textAlign: 'left', borderBottom: '2px solid #ddd' }}>Car (Brand/Model)</th>
        ...
        {showModal && (
          <div style={{
            ...
            <div style={{
              ...
              <h2 style={{ textAlign: 'center', marginBottom: '20px' }}>Modify reservation</h2>
              ...
              {selectedReservation && (
                <div style={{
                  ...
                  <strong>Current informations:</strong>
                  <div> Car: {selectedReservation.brand ? `${selectedReservation.brand} ${selectedReservation.model}` : `ID: ${selectedReservation.vehicle_id}`}</div>
                  <div> Pickup date: {new Date(selectedReservation.pickup_date).toLocaleDateString('hu-HU')}</div>
                  <div> Return date: {new Date(selectedReservation.return_date).toLocaleDateString('hu-HU')}</div>
                  <div> State: {selectedReservation.status || 'Foglalva'}</div>
                </div>
              )}
